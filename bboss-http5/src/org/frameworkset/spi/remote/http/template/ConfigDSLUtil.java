...
import org.frameworkset.spi.remote.http.serial.CustomDSLCharEscapeUtil;
...
public class ConfigDSLUtil {
	protected DslTemplateContainer templatecontext;
	...
	protected DslTemplateCache templateCache ;
	protected Map<String, DslInfo> esInfos;
...
//	protected  Map<String,ESUtil> esutils;
    ...
    private static DslTemplateFactory factory = null;
    static {
    ...
    private static void initESTemplateFactory2(){
        ...
        factory = new DslTemplateFactoryImpl2();
    ...
    public static DslTemplate createESTemplate(DslInfo dslInfo){
        return factory.createESTemplate(dslInfo);
    }
	public VariableHandler.URLStruction getTempateStruction(DslInfo dslInfo, String template) {
		return this.templateCache.getTemplateStruction(dslInfo,template);
	}

	public DslTemplateContainer getTemplatecontext() {
...
//							charset = column.charset();
							...
							if (cv == null)
								throw new DslConfigException("转换属性[" + beanInfo.getClazz().getName() + "." + property.getName() + "]值失败：值为null时，转换器必须返回ColumnType类型的对象,用来指示表字段对应的java类型。");
...
//				throw new DslConfigException(e.getTargetException());
//			}
...
//			dateFormateMeta = DateFormateMeta.buildDateFormateMeta(format);
			dateFormateMeta = DSLSerialUtil.getDateFormateMeta();
		}
	...
	public void handleVaribleValue(StringBuilder builder, DslTemplateCache.TempateVariable variable, String value, boolean escape){
		...
		if(variable.getLpad() != null || variable.getRpad() != null) {
			...
			if (!escape) {
				...
				if(escapeCount <= 1){
					DSLCharEscapeUtil DSLCharEscapeUtil = new CustomDSLCharEscapeUtil(new BBossStringWriter(builder),variable.getEsEncode());
					DSLCharEscapeUtil.writeString(innerValue.toString(), true);
				}
				else{
					String innerValueString = innerValue.toString();
					innerValue.setLength(0);
					for(int i = 0; i < escapeCount; i ++) {
						DSLCharEscapeUtil DSLCharEscapeUtil = new CustomDSLCharEscapeUtil(new BBossStringWriter(innerValue),variable.getEsEncode());
						DSLCharEscapeUtil.writeString(innerValueString, true);
...
//			builder.append("\"");
	...
	public void getVariableValue(StringBuilder builder, DslTemplateCache.TempateVariable variable, Object bean, List<ClassUtil.PropertieDescription> attributes, ClassUtil.ClassInfo beanInfo, String template) {
//		ClassUtil.ClassInfo beanInfo = ClassUtil.getClassInfo(bean.getClass());
		String name = null;
...
//							charset = column.charset();
							...
							Object cv = editor.toColumnValue(column, value);
							if (cv == null)
								throw new DslConfigException(new StringBuilder().append("Transform property[" )
...
//					params.addSQLParamWithDateFormateMeta(name, value, sqltype, dataformat,charset);
				...
				throw new DslConfigException(new StringBuilder().append("Failed to convert attribute values: Check template definitions").append("[")
...
//			}
		...
		throw new DslConfigException(new StringBuilder().append(beanInfo.getClazz().getName()).append("No value are specified for variable").append("[").append(variable.getVariableName()).append("] of the  dsl template[")
	...
	public void evalStruction(StringBuilder builder,VariableHandler.URLStruction templateStruction,Object bean,String template){
		...
		for(int i = 0; i < tokens.size(); i ++){
			...
			if(i < variables.size()) {
				DslTemplateCache.TempateVariable variable = (DslTemplateCache.TempateVariable)variables.get(i);
			...
			if(i < variables.size()) {
				...
				if (data == null) {
					if (bean.containsKey(variable.getVariableName()))
						...
						throw new DslConfigException(new StringBuilder()
								.append("No value are specified for variable")
								.append("[")
								.append(variable.getVariableName()).append("] of the  dsl template[")
								.append(template).append("]@").append(this.templatecontext.getNamespace())
								.append(" Error dsl:\r\n")
...
//							}
	...
	private  void handleObject(DslTemplateCache.TempateVariable variable, StringBuilder builder, Object value){
		if(variable.getSerialJson() == null
			...
			int escapeCount = variable.getEscapeCount();
			if(escapeCount <= 1) {
				builder.append(DSLSerialUtil.object2json(value));
			}
			else{
				String _value = DSLSerialUtil.object2json(value);
				StringBuilder innerValue = new StringBuilder();
				for(int i = 0; i < escapeCount - 1; i ++) {
					DSLCharEscapeUtil DSLCharEscapeUtil = new CustomDSLCharEscapeUtil(new BBossStringWriter(innerValue),variable.getEsEncode());
					DSLCharEscapeUtil.writeString(_value, true);
...
//		this.esInfos= null;
//		this.esrefs = null;
		Map<String, DslInfo> esInfos = new HashMap<String, DslInfo>();
		...
		if(keys != null && keys.size() > 0)
		{
			...
			while(keys_it.hasNext())
			{
				...
				DslTemplateMeta pro = this.templatecontext.getProBean(key);
				...
				if(templateFile == null)
				{
					...
					if(o instanceof String)
					{
						...
						if(value != null)
						{
                            ...
                            DslTemplate sqltpl = null;
							value = ConfigDSLUtil.ltrim(value);
							DslInfo sqlinfo = new DslInfo(key, value, istpl,multiparser,pro, cache);
	...
	void buildTemplateDatas(DslTemplateContainer newDslTemplateContainer){
		this.templatecontext = newDslTemplateContainer;
		trimValues();
		...
		templateCache = new DslTemplateCache(perKeyDSLStructionCacheSize,alwaysCacheDslStruction);
		destroyed = false;
	}
	void reinit()
	{
...
//		this.esutils = esutils;
		this.templateFile = templatefile;
		this.dslMappingDir = dslMappingDir;
		DslSOAFileApplicationContext dslSOAFileApplicationContext = new DslSOAFileApplicationContext(configHolder,dslMappingDir,templatefile);
		dslSOAFileApplicationContext.initProviderManager();
		this.templatecontext = new AOPDslTemplateContainerImpl(configHolder,dslMappingDir, dslSOAFileApplicationContext);
		this.perKeyDSLStructionCacheSize = templatecontext.getPerKeyDSLStructionCacheSize();//("perKeyDSLStructionCacheSize",ESUtil.defaultPerKeyDSLStructionCacheSize);
		this.alwaysCacheDslStruction  = templatecontext.isAlwaysCacheDslStruction();//getBooleanProperty("alwaysCacheDslStruction",ESUtil.defaultAlwaysCacheDslStruction);
		templateCache = new DslTemplateCache(perKeyDSLStructionCacheSize,alwaysCacheDslStruction);
...
//		checkESUtil();
	...
	public ConfigDSLUtil(ConfigHolder configHolder, DslTemplateContainer dslTemplateContainer) {
//		this.templateFile = templatefile;
//		this.esutils = esutils;
		this.configHolder = configHolder;
		this.templatecontext = dslTemplateContainer;//new AOPTemplateContainerImpl(this,new ESSOAFileApplicationContext(templatefile));
		this.perKeyDSLStructionCacheSize = templatecontext.getPerKeyDSLStructionCacheSize();//("perKeyDSLStructionCacheSize",ESUtil.defaultPerKeyDSLStructionCacheSize);
		this.alwaysCacheDslStruction  = templatecontext.isAlwaysCacheDslStruction();//getBooleanProperty("alwaysCacheDslStruction",ESUtil.defaultAlwaysCacheDslStruction);
		templateCache = new DslTemplateCache(perKeyDSLStructionCacheSize,alwaysCacheDslStruction);
		this.realTemplateFile = this.templatecontext.getNamespace();//.getConfigfile();
...
//	}
	...
	private DslInfo getReferESInfo(String templateName)
	{
		if(assertDestoried())
			return null;
	...
	public DslInfo getESInfo(String templateName) {
		if(templateName == null || templateName.equals("")){
			throw new DslConfigException("Dsl templateName cann't been empty or null.");
		}
		if(assertDestoried())
			return null;
		DslInfo sql = null;
	...
	public String getTemplate( String templateName) {
		...
		DslInfo dslInfo = esInfos != null?
					esInfos.get(templateName):null;
		 
		 	
		return dslInfo != null? dslInfo.getTemplate():null;
	...
	public String getTemplate( String templateName,Map variablevalues) {
		...
		String newsql = null;
		DslInfo sql =  this.esInfos != null ?this.esInfos.get(templateName):null;
	...
	public static String _getTemplate(DslInfo sqlinfo, Map variablevalues)
	{
//		String newsql = null;
//		if(sqlinfo.istpl() )
//		{
