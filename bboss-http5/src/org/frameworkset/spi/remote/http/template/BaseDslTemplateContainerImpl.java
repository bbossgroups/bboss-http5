 ...
 * @version 1.0
 */
public abstract class BaseDslTemplateContainerImpl implements DslTemplateContainer {
	private static Logger logger = LoggerFactory.getLogger(BaseDslTemplateContainerImpl.class);
	protected String namespace;
	protected Map<String, DslTemplateMeta> templateMetas;
	protected ConfigHolder configHolder;
	public BaseDslTemplateContainerImpl(String namespace) {
	 ...
	 * @return
	 */
	protected  abstract Map<String, DslTemplateMeta> loadTemplateMetas(String namespace);
	/**
	...
	private Map<String, DslTemplateMeta> _loadTemplateMetas(String namespace){
		final Map<String, DslTemplateMeta> templateMetaMap = loadTemplateMetas(namespace);
		if(templateMetaMap != null && templateMetaMap.size() > 0){
			//对dsl模板进行宏变量替换和特殊字符处理
			Iterator<Map.Entry<String, DslTemplateMeta>> iterator = templateMetaMap.entrySet().iterator();
			final PropertiesContainer configProperties = new PropertiesContainer();
			final DslServiceProviderManager dslServiceProviderManager = new DslServiceProviderManager(configHolder);
			final ValueContainer valueContainer = new ValueContainer(){
				...
				public String getMacroVariableValue(List<String> parentLinks,String text) {
					BaseDslTemplateMeta inTemplateMeta = (BaseDslTemplateMeta)templateMetaMap.get(text);
					if(inTemplateMeta != null){
						if(inTemplateMeta.isParsered())
							return (String)inTemplateMeta.getDslTemplate();
						else{ //递归分析引用片段

							evalValue(parentLinks,inTemplateMeta, configProperties, dslServiceProviderManager,this );
			...
			while(iterator.hasNext()){
				Map.Entry<String, DslTemplateMeta> templateMetaEntry = iterator.next();
				BaseDslTemplateMeta templateMeta = (BaseDslTemplateMeta)templateMetaEntry.getValue();
				if(templateMeta.isParsered()){
					continue;
				}
				parentLinks = new ArrayList<String>();
				parentLinks.add(templateMeta.getName());
				evalValue(parentLinks,templateMeta, configProperties, dslServiceProviderManager,valueContainer );
			}
		}
	...
	public void evalValue(List<String> parentLinks, BaseDslTemplateMeta templateMeta, PropertiesContainer configProperties, AOPValueHandler valueHandler, ValueContainer valueContainer)
	{
	...
	private synchronized void init(){
		if(templateMetas == null){
			...
			if(templateMetas == null){
				templateMetas = new HashMap<String, DslTemplateMeta>(0);
			}
		}
	...
	public synchronized void reinit(ConfigDSLUtil configDSLUtil){
		try {
			Map<String, DslTemplateMeta> temp = this._loadTemplateMetas(namespace);
			if(temp != null){
