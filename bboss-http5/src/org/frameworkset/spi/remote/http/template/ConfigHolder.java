...
public class ConfigHolder {
	...
	public ConfigDSLUtil getConfigDSLUtil(String templateFile){
		return getConfigDSLUtil(org.frameworkset.spi.remote.http.ClientConfiguration.getDslfileMappingDir(),templateFile);
//        return getConfigDSLUtil(dslfileMappingDir,templateFile);
        
	}
	public ConfigDSLUtil getConfigDSLUtil(DslTemplateContainer dslTemplateContainer) {
		String namespace = dslTemplateContainer.getNamespace();
		...
		synchronized(configDSLUtils)
		{
			...
			if(sqlUtil != null)
				return sqlUtil;
			sqlUtil = new ConfigDSLUtil(this, dslTemplateContainer);
...
//			e.printStackTrace();
	...
	private void checkESUtil(ConfigDSLUtil configDSLUtil){
		DslTemplateContainer dslTemplateContainer = configDSLUtil.getTemplateContext();
        if(!dslTemplateContainer.needMonitor()){
            if(log.isDebugEnabled())
                log.debug("{} needMonitor[false], ignore hotload DSL [{}]",holderName, dslTemplateContainer.getNamespace());
            return;
        }
		Long refresh_interval = ClientConfiguration.getDslfileRefreshInterval();
		if(refresh_interval != null && refresh_interval > 0)
		{
			...
			dslTemplateContainer.monitor(damon,new ConfigDSLUtil.ResourceTempateRefresh(configDSLUtil));
//			damon.addFile(fileUrl,templateNamespace, new ResourceTempateRefresh(sqlutil));
		}
		else{
            if(log.isDebugEnabled())
                log.debug("{} DSL Refresh Interval[{}], ignore hotload DSL [{}]",holderName,refresh_interval, dslTemplateContainer.getNamespace());
		}
