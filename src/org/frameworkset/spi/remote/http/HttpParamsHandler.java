...
public class HttpParamsHandler {
    ...
    public static Exception getException(HttpClientResponseHandler responseHandler, HttpServiceHosts httpServiceHosts ){
...
//        }
    ...
    public static List<NameValuePair> paramsPaires(Object params) throws HttpProxyRequestException {
         
        return paramsPaires(   params,null) ;
    }

    public static List<NameValuePair> paramsPaires(Object params,DataSerialType dataSerialType) throws HttpProxyRequestException {
        List<NameValuePair> pairs = null;
        if (params != null) {
            if(params instanceof Map){
                pairs =  mapParamsPairs(  (Map)params, dataSerialType);
            }
            else{
                try {
                    pairs =  objectParamsPairs(  params,dataSerialType);
                } catch (InvocationTargetException e) {
                    throw new HttpProxyRequestException(e);
                } catch (IllegalAccessException e) {
                    throw new HttpProxyRequestException(e);
...
//        }
    ...
    private static List<NameValuePair> objectParamsPairs(HttpRequestProxy.HttpOption httpOption) throws InvocationTargetException, IllegalAccessException {
        return objectParamsPairs(httpOption.params,httpOption.dataSerialType);


    }

    private static List<NameValuePair> objectParamsPairs(Object params,DataSerialType dataSerialType) throws InvocationTargetException, IllegalAccessException {
        ...
        if (params != null) {
            ...
            for(ClassUtil.PropertieDescription propertieDescription: propertieDescriptions) {
                ...
                if(value_ == null)
                    continue;
                String value = convertValue(value_,propertieDescription,dataSerialType);
...
//                }
    ...
    private static List<NameValuePair> mapParamsPairs(HttpRequestProxy.HttpOption httpOption){
        return mapParamsPairs((Map)httpOption.params,httpOption.dataSerialType);
    }

    private static List<NameValuePair> mapParamsPairs(Map params,DataSerialType dataSerialType){
        if(params.size() <= 0)
            return null;
        List<NameValuePair> paramPair = new ArrayList<NameValuePair>();
        ...
        for (int i = 0; it.hasNext(); i++) {
            ...
            if(value_ == null)
                continue;
            String value = convertValue(value_,dataSerialType);
            String name = String.valueOf(entry.getKey());
            paramPair_ = new BasicNameValuePair(name, value);
//            if(value instanceof String) {
//                paramPair_ = new BasicNameValuePair(name, String.valueOf(value));
...
//            }
    ...
    private static void objectHttpParams(URIBuilder uriBuilder,Object params) {
//        if (params != null && params.size() > 0) {
...
//                }
    ...
    public static URI httpParams(String url,Object params) throws URISyntaxException {
        return uriBuilder(  url,   params);
         

    }
    
    
    public static URI uriBuilder(String url, Object params) throws URISyntaxException {
        URIBuilder uriBuilder = new URIBuilder(url);
        if(params instanceof Map){
            mapHttpParams(uriBuilder,(Map)params);
            
        }
        else{
            objectHttpParams(uriBuilder,params);
        }
        return uriBuilder.build();
    }
    private static void mapHttpParams(URIBuilder uriBuilder,Map params){
        if(params.size() <= 0)
            return ;
